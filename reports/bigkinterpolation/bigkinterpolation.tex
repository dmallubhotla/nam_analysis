...
\begin{document}
	...
	\begin{figure}[htp]
		\centering
		\includegraphics[width=12cm]{chiZEmediumboi}
	...
	As seen in \fref{fig:chiZMedium}, over more physical values of $z$, the calculated noise varies even for physically relevant $z$, over the range of plausible values of $u_{uc}$, which suggests that a better approach is needed.
	...
	If we correct our math mistakes we see that \fref{fig:chiZEarounduF} is the one that's actually correct.\todo{remove pointless bad plots}
