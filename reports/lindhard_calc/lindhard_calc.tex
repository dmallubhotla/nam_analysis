...
\begin{document}
	...
	In \fref{fig:electriccutoffs} we can see what happens as we vary the cutoff frequency while calculating the cutoffs.
	The cutoff has no effect on the noise.
	\begin{figure}[htp]
		\centering
		\includegraphics[width=12cm]{chiZELindhard}
		\caption{$\chi_{EE}(z)$, with $z$ in units of $\flatfrac{\omega}{c}$, for different integration cutoffs} \label{fig:electriccutoffs}
	\end{figure}
